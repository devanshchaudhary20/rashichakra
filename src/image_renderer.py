...
def render_zodiac_slide(
    ...
    """Overlay the zodiac name + daily horoscope on the user's template image."""
    base = Image.open(template_path).convert("RGB")
    base = _fit_to_canvas(base).convert("RGBA")

    # Subtle dark panel behind text in the lower half so any background works.
    panel_top = int(config.IMG_HEIGHT * config.HOROSCOPE_TEXT_TOP_FRAC) - 40
    panel = Image.new("RGBA", (config.IMG_WIDTH, config.IMG_HEIGHT), (0, 0, 0, 0))
    pdraw = ImageDraw.Draw(panel)
    pdraw.rectangle(
        [(0, panel_top), (config.IMG_WIDTH, config.IMG_HEIGHT)],
